...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		//调用
		//如果key为null，只需要找到最左边的left，
		// 具体操作为判断当前是否为intel是则向左找，直到找到子页
		//如果对于一般，则格局entry判断是否是在左子夜，如果一直不是则找右边子子叶

		//这里是递归调用，结束的点为找到子页面，同时我们可以认为只有intel和left page

		//先写出递归终点
		//根据提示pgcateg获取类别，BTreeFile.getPage()获取页面
		...
		//判断是否为f==null,如果是直接找到最左边的left
		if (f == null){
			//不用while循环，直接找到第一个entry的leftChild
			if (iterator.hasNext()){
	...
	public BTreeLeafPage splitLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
		...
		while (reverseIterator.hasNext() && mid > 0){
			Tuple next = reverseIterator.next();
			page.deleteTuple(next);
			emptyPage.insertTuple(next);
			mid--;
		}
		dirtypages.put(emptyPage.getId(), emptyPage);

		Tuple up = reverseIterator.next();
		BTreeInternalPage parentPage = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), up.getField(keyField));
		BTreeEntry bTreeEntry = new BTreeEntry(up.getField(keyField), page.getId(), emptyPage.getId());
		...
		if (rightSiblingId != null){
			BTreeLeafPage bTreeLeafPage = (BTreeLeafPage) getPage(tid, dirtypages, rightSiblingId, Permissions.READ_WRITE);
			bTreeLeafPage.setLeftSiblingId(emptyPage.getId());
			dirtypages.put(rightSiblingId, bTreeLeafPage);
		}
		...
		emptyPage.setParentId(parentPage.getId());

		if (up.getField(keyField).compare(Op.GREATER_THAN_OR_EQ, field)) {
			return page;
		}
		return emptyPage;
	}
	...
	public BTreeInternalPage splitInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		//想法相同因为getParentWithEmptySlots会返回一个合理的slot，因此我们向上的时候不需要自己差分页面

		int mid = page.getMaxEntries() / 2;
		...
		while (reverseIterator.hasNext() && mid > 0){
			...
			emptyPage.insertEntry(next);
			mid--;
		}
		dirtypages.put(emptyPage.getId(), emptyPage);
		dirtypages.put(page.getId(),page);

		BTreeEntry up = reverseIterator.next();
		page.deleteKeyAndRightChild(up);
		up.setLeftChild(page.getId());
		up.setRightChild(emptyPage.getId());
	...
	public void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		//首先计算一下要steal多少个
		int half = (page.getNumTuples() + sibling.getNumTuples()) / 2 - page.getNumTuples();
		//要从那边
	...
	public void stealFromLeftInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		int half = (leftSibling.getNumEntries() + page.getNumEntries()) / 2 - page.getNumEntries();
		Iterator<BTreeEntry> reverseIterator = leftSibling.reverseIterator();
		Iterator<BTreeEntry> iterator = page.iterator();
		BTreeEntry curFirst = iterator.next();
		BTreeEntry leftLast = reverseIterator.next();
		page.insertEntry(new BTreeEntry(parentEntry.getKey(), leftLast.getRightChild(), curFirst.getLeftChild()));
		if (half == 1){
			leftSibling.deleteKeyAndLeftChild(leftLast);
			parentEntry.setKey(leftLast.getKey());
			parent.updateEntry(leftLast);
		...
		BTreeEntry up = reverseIterator.next();
		leftSibling.deleteKeyAndRightChild(up);
		parentEntry.setKey(up.getKey());
	...
	public void stealFromRightInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		BTreeEntry up = bTreeEntryIterator.next();
		rightSibling.deleteKeyAndLeftChild(up);
		parentEntry.setKey(up.getKey());
		parent.updateEntry(parentEntry);
		updateParentPointers(tid, dirtypages, page);
		dirtypages.put(parent.getId(),parent);
	...
	public void mergeLeafPages(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		if (rightPage.getRightSiblingId() != null) {
			BTreeLeafPage bTreeLeafPage = (BTreeLeafPage) getPage(tid, dirtypages, rightPage.getRightSiblingId(), Permissions.READ_WRITE);
			bTreeLeafPage.setLeftSiblingId(leftPage.getId());
			dirtypages.put(bTreeLeafPage.getId(), bTreeLeafPage);
		}
		while (iterator.hasNext()) {
	...
	public void mergeInternalPages(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		Iterator<BTreeEntry> iterator1 = leftPage.reverseIterator();
		BTreeEntry first = iterator.next();
		BTreeEntry last = iterator1.next();
		BTreePageId rightChild = first.getLeftChild();
		BTreePageId leftChild = last.getRightChild();
		BTreeEntry insetEntry = new BTreeEntry(parentEntry.getKey(),leftChild,rightChild);
		leftPage.insertEntry(insetEntry);
